...
router.post("/claim-rewards", async (req, res) => {
  ...
  const providedComboCode =
    typeof req.body?.comboCode === "string" ? req.body.comboCode.trim() : "";

  try {
    const result = await claimRewardCombo(telegramId, providedComboCode, {
      username: req.tgUser.username || null,
      name:
        [req.tgUser.first_name, req.tgUser.last_name]
          .filter(Boolean)
          .join(" ") || null,
    });
...
router.post("/send-money", async (req, res) => {
  ...
  try {
    ...
    if (
      senderPhoneVariants.some((phone) => receiverPhoneVariants.includes(phone))
    ) {
      return res
        .status(400)
        .json({ error: "You cannot send money to your own phone number" });
    }
    ...
    if (!receiver) {
      return res
        .status(404)
        .json({ error: "Receiver not found for this phone number" });
    }
    ...
    if (err?.status === 400) {
      return res
        .status(400)
        .json({ error: err.message || "Insufficient balance" });
    }
