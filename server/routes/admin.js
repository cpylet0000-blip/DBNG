...
// Admin: SpinWin round summary (latest rounds with profit/loss)
router.get('/spin-win/rounds', adminAuthMiddleware, async (req, res) => {
  try {
    const limitRaw = Number.parseInt(String(req.query.limit || '20'), 10)
    const limit = Number.isFinite(limitRaw) ? Math.min(Math.max(limitRaw, 1), 100) : 20

    // Fetch enough recent spin rows to reconstruct round snapshots by second.
    const recentSpins = await prisma.spinWinSpin.findMany({
      select: {
        id: true,
        createdAt: true,
        winningNumber: true,
      ...
      take: Math.max(limit * 80, 400),
    })

    const grouped = new Map()
    for (const spin of recentSpins) {
      const key = toSecondKey(spin.createdAt)
      const existing = grouped.get(key)
      if (!existing) {
        grouped.set(key, {
          roundId: spin.id,
          roundAt: truncateToSecond(spin.createdAt),
          winningNumber: spin.winningNumber,
          winningColor: spin.winningColor,
          participantsSet: new Set([spin.userId]),
        })
      } else {
        existing.participantsSet.add(spin.userId)
        if (spin.id < existing.roundId) {
          existing.roundId = spin.id
        }
      }

      // Keep one extra round to calculate previous window for the oldest displayed round.
      if (grouped.size >= limit + 1) {
        const newestKey = Array.from(grouped.keys())[0]
        if (key !== newestKey) {
          break
        }
      }
    ...
    const roundsDesc = Array.from(grouped.values())
    ...
    const displayRounds = roundsDesc.slice(0, limit).map((r, idx) => ({
      roundId: r.roundId,
      roundAt: r.roundAt,
      prevRoundAt: roundsDesc[idx + 1]?.roundAt || null,
      winningNumber: r.winningNumber,
      winningColor: r.winningColor,
      participants: r.participantsSet.size,
      totalBets: 0,
      totalWagered: 0,
      totalPaid: 0,
      profit: 0,
    ...
    const newestRoundAt = displayRounds[0].roundAt
    const oldestPrevRoundAt = displayRounds[displayRounds.length - 1].prevRoundAt

    const betsInRange = await prisma.spinWinBet.findMany({
      where: {
        status: { in: ['won', 'lost'] },
        createdAt: {
          lte: newestRoundAt,
          ...(oldestPrevRoundAt ? { gt: oldestPrevRoundAt } : {}),
        },
      },
      select: {
        createdAt: true,
    ...
    for (const bet of betsInRange) {
      ...
      for (const round of displayRounds) {
        const upper = round.roundAt.getTime()
        const lower = round.prevRoundAt ? round.prevRoundAt.getTime() : -Infinity
        if (t <= upper && t > lower) {
          round.totalBets += 1
          round.totalWagered += Number(bet.amount || 0)
          round.totalPaid += Number(bet.winnings || 0)
          break
        }
      }
    }

    const mapped = displayRounds.map((r) => ({
      ...r,
      profit: r.totalWagered - r.totalPaid,
    }))

    res.json({ success: true, rounds: mapped })
  } catch (error) {
    console.error('Failed to load SpinWin rounds:', error)
    res.status(500).json({ success: false, error: 'Failed to load SpinWin rounds' })
  }
})

// Admin: SpinWin summary metrics (daily/weekly/monthly)
router.get('/spin-win/summary', adminAuthMiddleware, async (req, res) => {
  try {
    const now = new Date()
    ...
    const mapSummary = (row) => {
      const totalWagered = Number(row?._sum?.amount || 0)
      const totalPaid = Number(row?._sum?.winnings || 0)
      return {
        totalBets: Number(row?._count || 0),
        totalWagered,
        totalPaid,
    ...
    return res.json({
      success: true,
      summary: {
        daily: mapSummary(daily),
        weekly: mapSummary(weekly),
        monthly: mapSummary(monthly),
      },
    })
  } catch (error) {
    console.error('Failed to load SpinWin summary:', error)
    return res.status(500).json({ success: false, error: 'Failed to load SpinWin summary' })
  }
})

// Admin: SpinWin round details (who bet what on a selected round)
router.get('/spin-win/round-details', adminAuthMiddleware, async (req, res) => {
  try {
    const roundAtRaw = String(req.query.roundAt || '').trim()
    if (!roundAtRaw) {
      return res.status(400).json({ success: false, error: 'roundAt is required' })
    }

    const roundAt = new Date(roundAtRaw)
    if (Number.isNaN(roundAt.getTime())) {
      return res.status(400).json({ success: false, error: 'roundAt is invalid' })
    }

    const normalizedRoundAt = truncateToSecond(roundAt)

    const previousSpin = await prisma.spinWinSpin.findFirst({
      where: { createdAt: { lt: normalizedRoundAt } },
      orderBy: { createdAt: 'desc' },
      select: { createdAt: true },
    })

    const prevRoundAt = previousSpin?.createdAt ? truncateToSecond(previousSpin.createdAt) : null

    const [roundSpins, bets] = await Promise.all([
      prisma.spinWinSpin.findMany({
        where: {
          createdAt: {
            gte: normalizedRoundAt,
            lt: new Date(normalizedRoundAt.getTime() + 1000),
          },
        },
        orderBy: { id: 'asc' },
        select: {
      ...
      prisma.spinWinBet.findMany({
        where: {
          createdAt: {
            gt: prevRoundAt ? new Date(prevRoundAt) : new Date(0),
            lte: normalizedRoundAt,
          },
          status: { in: ['won', 'lost'] },
        },
        orderBy: { createdAt: 'desc' },
    ...
    const winningNumber = roundSpins[0]?.winningNumber ?? null
    const winningColor = roundSpins[0]?.winningColor ?? null
    const roundId = roundSpins[0]?.id ?? null

    const totalWagered = bets.reduce((sum, b) => sum + Number(b.amount || 0), 0)
    const totalPaid = bets.reduce((sum, b) => sum + Number(b.winnings || 0), 0)
    ...
    console.error('Failed to load SpinWin round details:', error)
    res.status(500).json({ success: false, error: 'Failed to load SpinWin round details' })
  }
})
