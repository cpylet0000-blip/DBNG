...
export default function RequestsPage() {
  ...
  const calculateDepositSums = (deposits: DepositReq[]) => {
    ...
    for (const d of deposits) {
      ...
      if (created.getMonth() === now.getMonth() && created.getFullYear() === now.getFullYear()) month += d.amount;
    }
  ...
  const filteredDeposits = useMemo(() => {
    let items = deposits;
    if (statusFilter !== 'all') items = items.filter(r => r.status.toLowerCase() === statusFilter);
  ...
  const depositTotalPages = Math.max(1, Math.ceil(filteredDeposits.length / pageSize));
  ...
  const withdrawTotalPages = Math.max(1, Math.ceil(filteredWithdrawals.length / pageSize));
  ...
  const statusCounts = allTx.reduce((acc, tx) => {
        ...
        <div className="w-full max-w-6xl flex flex-col gap-6">
          <header className="flex flex-col sm:items-end sm:justify-between gap-3">
            ...
            <div className="flex flex-wrap items-center gap-2 text-xs text-neutral-500 mt-2">
              ...
              <select
                value={kindFilter}
                onChange={(e) => setKindFilter(e.target.value as 'all' | 'deposit' | 'withdrawal')}
            ...
            <div className="flex flex-wrap gap-2 mt-2">
              {Object.entries(statusCounts).map(([status, count]) => (
                <span key={status} className="px-2 py-1 rounded bg-neutral-100 text-xs text-neutral-700 border border-neutral-200">
              ...
              <div className="bg-white rounded-lg border border-neutral-200 p-4">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                    ...
                    <h2 className="text-sm font-medium text-neutral-900">Deposits</h2>
                  </div>
                  <span className="text-[11px] text-neutral-500">
                    {loading ? 'Loading...' : `${filteredDeposits.length} item(s)`}
                  </span>
                </div>
                      ...
                      <div className="flex flex-col">
                        <span className="text-sm text-neutral-800">
                          {req.user.name && req.user.username ? (
                            <>
                              {req.user.name}
                              <span className="ml-1 px-1 text-[10px] text-gray-700 bg-gray-100 rounded-full align-middle">@{req.user.username}</span>
                            </>
                          ) : req.user.name ? (
                            req.user.name
                          ) : req.user.username ? (
                            <span className="px-1 text-[10px] text-gray-700 bg-gray-100 rounded-full align-middle">@{req.user.username}</span>
                          ) : (
                        ...
                        <span className="text-[10px] uppercase tracking-wide text-green-500">
                          {req.paymentDateTime ? `Paid at: ${new Date(req.paymentDateTime).toLocaleString()}` : 'Not paid yet'}
                        </span>
                      </div>
                      <div className="flex flex-col items-end gap-1">
                        ...
                        {req.transactionId ? (
                          req.account == '251988818312' ? (
                            <a href={`https://cbepay1.cbe.com.et/aureceipt?TID=${req.transactionId}&PH=0${process.env.NEXT_PUBLIC_CBE_BIRR_ACCOUNT}`} target="_blank" rel="noopener noreferrer" className="text-[10px] uppercase tracking-wide text-blue-500">
                              {req.transactionId}
                            </a>
                           ) :
                          req.transactionId.length > 17 ? (
                            <a href={`https://apps.cbe.com.et:100/?id=${req.transactionId}`} target="_blank" rel="noopener noreferrer" className="text-[10px] uppercase tracking-wide text-blue-500">
                              {req.transactionId}
                            </a>
                          ) : req.transactionId.length >= 15 && req.transactionId.length <= 18 ? (
                            <a href={`https://cs.bankofabyssinia.com/slip/?trx=${req.transactionId}`} target="_blank" rel="noopener noreferrer" className="text-[10px] uppercase tracking-wide text-blue-500">
                              {req.transactionId}
                            </a>
                          ) : (
                            <a href={`https://transactioninfo.ethiotelecom.et/receipt/${req.transactionId}`} target="_blank" rel="noopener noreferrer" className="text-[10px] uppercase tracking-wide text-blue-500">
                              {req.transactionId}
                            </a>
                          )
                        ) : null}
                        <a href='' className="text-[10px] uppercase tracking-wide text-neutral-500">
                          {req.transactionId ? `TX: ${req.transactionId}` : 'No TX ID'}
                        </a>
                      </div>
                    </div>
                  ))}
              ...
              <div className="bg-white rounded-lg border border-neutral-200 p-4">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                    ...
                    <h2 className="text-sm font-medium text-neutral-900">Withdrawals</h2>
                  </div>
                  <span className="text-[11px] text-neutral-500">
                    {loading ? 'Loading...' : `${filteredWithdrawals.length} item(s)`}
                  </span>
                </div>
                <div className="space-y-2 max-h-105 overflow-y-auto pr-1">
