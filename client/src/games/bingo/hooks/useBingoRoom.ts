...
export function useBingoRoom() {
      ...
      if (
        ...
        try {
          ...
          const response = await fetch(
            `${API_BASE}/api/bingo/session/?room=${roomNumber}`,
            {
  ...
  const selectStake = useCallback(
    async (
      stake: BingoStake,
      roomNumber: number = gameState.selectedRoom ?? 1,
    ) => {
      ...
      console.log("userId:", userId);
      console.log(
        "Current gameState.myCards length:",
        gameState.myCards.length,
      );
      ...
      try {
        ...
        if (sessionData.success && cardsData.success) {
          ...
          if (sessionData.session && sessionData.session.players && userId) {
            const userPlayers = sessionData.session.players.filter(
              (p: SessionPlayer) => p.userId === userId,
            );
          ...
          console.error(
            ...
            sessionData.error || cardsData.error,
          );
        }
      } catch (error) {
  ...
  const selectMultipleCards = useCallback(
    async (cardIds: number[]) => {
      ...
      try {
        ...
        const tgData = (headers as Record<string, string>)[
          "x-telegram-init-data"
        ];
        const hasAuth = Boolean(devId || tgData);
        if (!hasAuth) {
          alert(
            "Unauthorized: missing Telegram context or dev ID. Please start via Telegram or set VITE_DEV_TELEGRAM_ID.",
          );
          return;
        }

        const response = await fetch(
          `${API_BASE}/api/bingo/select-multiple-cards`,
          {
            method: "POST",
            headers,
            body: JSON.stringify({
        ...
        if (data.success) {
          ...
          setGameState((prev) => {
            ...
            const existingCardIds = new Set(
              prev.myCards.map((card) => card.id),
            );
            const newCardIds = new Set(
              newCards.map((card: PlayerResponse) => card.id),
            );

            // Keep existing cards that weren't just selected (preserve their marked cells)
            const existingCardsToKeep = prev.myCards.filter(
              (card) => !newCardIds.has(card.id),
            );

            // For cards that already exist, preserve their marked cells
            const mergedCards = newCards.map(
              (newCard: {
                id: number;
                numbers: number[];
                markedCells: number[];
              }) => {
                const existingCard = prev.myCards.find(
                  (c) => c.id === newCard.id,
                );
                if (existingCard) {
          ...
          const isAlreadySelectedError =
            data.error?.includes("already selected") ||
          ...
          if (isAlreadySelectedError) {
            ...
            try {
              ...
              if (sessionData.success && sessionData.session) {
                ...
                if (myPlayerCards.length > 0) {
                  setGameState((prev) => {
                    ...
                    const newCardIds = new Set(
                      myPlayerCards.map((card: { id: number }) => card.id),
                    );
                    ...
                    // For cards that already exist, preserve their marked cells
                    const mergedCards = myPlayerCards.map(
                      (newCard: {
                        id: number;
                        numbers: number[];
                        markedCells: number[];
                    ...
                    return {
                      ...
                      selectedRoom:
                        sessionData.session?.roomNumber ?? prev.selectedRoom,
                      view:
                        sessionData.session.status === "active"
                          ? "game"
                          : "game",
              ...
              console.error(
                "Error fetching session for already-selected cards:",
                fetchError,
              );
            }
          }
    ...
    const pollSession = async () => {
      try {
        const res = await fetch(
          `${API_BASE}/api/bingo/session/${gameState.selectedStake}?room=${gameState.selectedRoom}`,
          {
            headers: buildHeaders(),
          },
        );
        ...
        if (!cancelled && data.success && data.session) {
          setGameState((prev) => {
            ...
            // ── Auto-dismiss from finished game view ──────────────────────
            // When polling gets a NEW session (different ID) while the player
            // is still on the "game" view from a round that just ended, the
            // WinnerDisplay may have already unmounted (because session.winner
            // became null in a previous poll), cancelling its 6-second auto-
            // close timer and leaving the player stuck on the watching-only
            // game screen. Detect this and navigate to card-select so they
            // can pick cards for the new round.
            if (
              isDifferentSession &&
              prev.view === "game" &&
              (prev.session?.status === "finished" || prev.session?.winner)
            ) {
              return {
                ...prev,
                session: data.session,
            ...
            // Safety-net: force view → "game" ONLY when the session just
            // transitioned from countdown/waiting → active (i.e. game just
            // started and the WebSocket event was missed).
            //
            // Crucially, if the user clicked the back arrow while a game was
            // already active, prev.session?.status is already "active" so
            // sessionBecameActive is false and they remain on card-select
            // (showing the "game in progress" overlay, screenshot 3).
            const prevStatus = prev.session?.status;
            const sessionBecameActive =
              data.session.status === "active" &&
              (prevStatus === "countdown" || prevStatus === "waiting");
            const viewIsStuck =
              prev.view === "card-select" || prev.view === "queue";
            const shouldForceGameView = sessionBecameActive && viewIsStuck;

            console.log(
              "[useBingoRoom] Poll: prevStatus=",
              prevStatus,
              "newStatus=",
              data.session.status,
              "view=",
            ...
            return {
              ...prev,
              session: data.session,
              myCards: isDifferentSession ? [] : prev.myCards,
              // Force view to game only when session just became active
              view: shouldForceGameView ? "game" : prev.view,
            };
          });
        }
      } catch (e) {
  ...
  const handlePlayerJoined = useCallback(
    (data: BingoWSEvents["bingo:player_joined"]) => {
  ...
  const handleCountdownStarted = useCallback(
    (data: BingoWSEvents["bingo:countdown_started"]) => {
      console.log("[useBingoRoom] Countdown started event received:", data);
      setGameState((prev) => {
        console.log(
          "[useBingoRoom] Countdown started - current myCards:",
          prev.myCards,
        );
        return {
          ...prev,
  ...
  const handleGameStarted = useCallback(
    (_data: BingoWSEvents["bingo:game_started"]) => {
      console.log("[useBingoRoom] Game started event received:", _data);
      setGameState((prev) => {
        console.log(
          "[useBingoRoom] Game started - current myCards:",
          prev.myCards,
        );
        return {
          ...prev,
  ...
  const handleBallDrawn = useCallback(
    (data: BingoWSEvents["bingo:ball_drawn"]) => {
      setGameState((prev) => {
        ...
        if (
          ...
          prev.session.calledNumbers.every(
            (n, i) => n === data.calledNumbers[i],
          )
        ) {
          return prev;
        }
  ...
  const handleAutoMarkUpdated = useCallback(
    (data: BingoWSEvents["bingo:automark_updated"]) => {
      setGameState((prev) => ({
        ...prev,
        autoMark: data.autoMark,
        myCards: prev.myCards.map((card) => ({
          ...card,
        ...
        setGameState((prev) => {
          ...
          if (isWinner) {
            ...
            if (data.multipleWinners && data.winners) {
              const myWin = data.winners.find(
                (w) =>
                  w.cardId === prev.myCards.find((c) => c.id === w.cardId)?.id,
              );
              if (myWin) {
              ...
              ? {
                ...
                winner: data.multipleWinners
                  ? {
                    ...
                    cardNumbers:
                      data.winners?.[0]?.cardNumbers || undefined,
                    pattern: data.winners?.[0]?.pattern || undefined,
                    winningCells:
                      data.winners?.[0]?.winningCells || undefined,
  ...
  const handleGameReset = useCallback(
    (data: BingoWSEvents["bingo:game_reset"]) => {
      setGameState((prev) => ({
        ...initialState,
        selectedStake: data.stake as BingoStake,
        // Always return to card-select after a game reset so the player can
        // pick cards for the next round. Previously this only went to
        // card-select if prev.view was already "card-select" — if the reset
        // event fired while the winner display was still showing ("game" view)
        // the player ended up on stake-select instead.
        view: "card-select",
  ...
  useEffect(() => {
    ...
    handleGameStarted,
    handleBallDrawn,
    handleAutoMarkUpdated,
    handleGameWon,
    handleGameReset,
  ]);

  const markCell = useCallback(
    (cardId: number, cellIndex: number) => {
      if (!gameState.selectedStake || !userId || gameState.myCards.length === 0)
        return;
  ...
  const leaveActiveGameAndSwitch = useCallback(
    async (newStake: BingoStake) => {
      if (!userId) return;

      try {
        const response = await fetch(
          `${API_BASE}/api/bingo/leave-active-game`,
          {
            method: "POST",
            headers: buildHeaders(),
          },
      ...
      selectStake(gameState.selectedStake, roomNum).then(() => {
        ...
        if (connected && userId) {
          ...
          setTimeout(async () => {
            try {
              const sessionRes = await fetch(
                `${API_BASE}/api/bingo/session/${gameState.selectedStake}?room=${roomNum}`,
                {
                  headers: buildHeaders(),
                },
              );
              ...
              if (sessionData.success && sessionData.session) {
                ...
                if (
                  sessionData.session.players.length >= 2 &&
                  sessionData.session.status === "waiting"
                ) {
                  console.log(
                    "[goToCardSelector] Triggering countdown check for session with",
                    sessionData.session.players.length,
                    "players",
                  );
                  emit("bingo:join_room", {
                    stake: gameState.selectedStake,
              ...
              console.error(
                "Error checking session after card selector navigation:",
                err,
              );
            }
          }, 500);
  ...
  // Function to navigate to card selector after game ends (winner display closed)
  // Also used by the back arrow in the game view.
  const goToStakeSelector = useCallback(() => {
    setGameState((prev) => ({
      ...prev,
      view: "card-select",
      // Clear finished-game cards so the user selects fresh for the next round
      myCards: [],
      // Keep the real session status intact so the polling loop does NOT treat
      // it as a waiting→active transition and redirect back to the game view.
      // Only clear winner/calledNumbers when the game is finished.
      session: prev.session
        ? {
          ...prev.session,
          winner: null,
          calledNumbers: prev.session.status === "finished" ? [] : prev.session.calledNumbers,
          countdownEndsAt: prev.session.status === "finished" ? null : prev.session.countdownEndsAt,
        }
        : null,
    }));
  }, []);
