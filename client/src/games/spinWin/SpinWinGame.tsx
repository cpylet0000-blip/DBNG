...
const SPIN_ANIMATION_MS = (SPIN_ANIMATION_DURATION_S * 1000) + SPIN_ANIMATION_BUFFER_MS
...
const SpinWinGame: React.FC = () => {
    ...
    const playSpinAudio = useCallback(() => {
        try {
            ...
            if (playPromise) {
                playPromise.catch(() => { /* browser blocked autoplay - ignore */ })
            }
    ...
    const selectedChipIndex = chipValues.findIndex((value) => value === selectedChip)
    const pendingPotentialReturn = pendingSelections.reduce((sum, bet) => sum + (bet.amount * bet.odds), 0)
    const ticketedPotentialReturn = ticketedBets.reduce((sum, bet) => sum + (bet.amount * bet.odds), 0)
    ...
    const showWheel = isSpinPhase || isSpinning || highlightedWinningNumber !== null || showWinnersDisplay
    ...
    const zeroTailStart = sectorStartAngle + (SECTOR_GROUPS.length * sectorSpanAngle)
    ...
    const loadGameData = useCallback(async () => {
        ...
        try {
            ...
            setTicketedBets((prev) => {
                const acceptedVeryRecently = Date.now() - lastTicketAcceptAtRef.current < 5000
                if (pendingBets.length === 0 && prev.length > 0 && acceptedVeryRecently) {
                    return prev
                }
                return pendingBets
            })

            const parsedHistory: SpinResult[] = (historyRes?.spins || []).map((spin) => ({
    ...
    useEffect(() => {
        ...
        const applyRoundStarted = async (data: SocketRoundStarted) => {
            ...
            if (spinningRoundGameIdRef.current && data.gameId !== spinningRoundGameIdRef.current) {
        ...
        const handleSpinResult = (data: SocketSpinResult) => {
            ...
            const newRotation = lastRotationRef.current + (360 * Math.floor(spins)) + delta
            ...
            window.setTimeout(async () => {
                ...
                const winnersDisplayDuration = hasWinners ? WINNING_NUMBER_HIGHLIGHT_MS + WINNER_DISPLAY_EXTENSION_MS : WINNING_NUMBER_HIGHLIGHT_MS

                // Hide highlighted number after highlight duration
                window.setTimeout(() => {
                    setHighlightedWinningNumber((current) => (current === winningNumber ? null : current))
        ...
        const handleRoundTimer = (data: SocketRoundTimer) => {
            ...
            if (typeof data?.roundEndsAt === 'number') {
                ...
                if (typeof currentRoundEndsAt === 'number' && data.roundEndsAt < currentRoundEndsAt) {
                    return
                }
    ...
    const placeBet = (type: string, value: string | number, odds: number) => {
        if (isSpinning) {
            setActionError('Spin is running. Wait for result before placing a new bet.')
            return
        }

        if (typeof spinCountdown === 'number' && spinCountdown <= 0) {
            setActionError('Betting window is closed. Wait for the next round countdown.')
            return
        }
        ...
        setPendingSelections((prev) => {
            const currentPendingStake = prev.reduce((sum, bet) => sum + bet.amount, 0)

            if (type === 'exact') {
                const existingExactIndex = prev.findIndex((bet) => bet.type === 'exact')
                ...
                if (existingExactIndex >= 0) {
                    ...
                    const selectedNumbers = String(existingExact.value)
                        ...
                        .filter((entry) => Number.isInteger(entry) && entry >= 0 && entry <= 36)

                    const exactNumber = Number(selectedValue)
                    if (!Number.isInteger(exactNumber) || exactNumber < 0 || exactNumber > 36) {
                        setActionError('Invalid exact number selected.')
                        return prev
                    }

                    if (selectedNumbers.includes(exactNumber)) {
                        const nextSelectedNumbers = selectedNumbers.filter((num) => num !== exactNumber)
                ...
                if (currentPendingStake + selectedChip > balance) {
                    setActionError('Insufficient balance for selected chip. Choose a lower chip.')
                    return prev
                }
            ...
            const existingIndex = prev.findIndex((bet) => bet.type === type && String(bet.value) === selectedValue)
            if (existingIndex >= 0) {
    ...
    const acceptPendingBets = async () => {
        ...
        const totalPendingStake = pendingSelections.reduce((sum, bet) => sum + bet.amount, 0)
        ...
        try {
            ...
            loadGameData().catch(err => console.error('Background refresh failed:', err))
            refresh()
        } catch (error) {
            ...
            loadGameData().catch(err => console.error('Recovery refresh failed:', err))
            refresh()
        }
    }
            ...
            <div className="w-full flex-1 flex flex-col pt-16 min-h-0 overflow-y-auto overflow-x-hidden pb-4">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,2.2fr)_minmax(340px,1fr)] gap-6 flex-1 px-1 sm:px-4">
                    <div className={`space-y-6 flex flex-col w-full ${showWheel ? 'm-auto' : ''}`}>
                        {showWheel && (
                            <div className="bg-slate-800 rounded-xl p-6 shadow-2xl">
                                ...
                                <div className="relative w-full max-w-[620px] aspect-square mx-auto mb-2 overflow-hidden">
                                    ...
                                    {/* Top indicator arrow */}
                                    <div className="absolute left-1/2 top-0 z-40" style={{ transform: 'translate(-50%, -45%)' }}>
                                        <div className="relative">
                                            <div className="absolute -inset-2 bg-yellow-400/40 rounded-full blur-md animate-pulse"></div>
                                    ...
                                    >
                                        {SECTOR_GROUPS.map((sector) => {
                                            const angle = ((sector.startWheelIndex - 0.5) * 360) / WHEEL_NUMBERS.length
                                            return (
                                                <div
                                        ...
                                        <div
                                            className="absolute inset-0"
                                            style={{ transform: `rotate(${((WHEEL_NUMBERS.length - 0.5) * 360) / WHEEL_NUMBERS.length}deg)` }}
                                        >
                                            <div className="absolute left-1/2 top-1.5 w-1 h-[43%] bg-amber-200/90 -translate-x-1/2 origin-top" />
                                        </div>
                                        ...
                                        {WHEEL_NUMBERS.map((num, index) => {
                                            ...
                                            return (
                                                <div key={num} className="absolute inset-0" style={{ transform: `rotate(${angle}deg)` }}>
                                                    <div
                                                        className={`absolute left-1/2 top-1.5 -translate-x-1/2 w-5 sm:w-6 h-10 sm:h-12 border border-yellow-700/70 shadow-lg flex items-start justify-center pt-1 transition-all duration-300 ${color === 'red'
                                                            ? 'bg-red-600'
                                                            : color === 'black'
                                                                ? 'bg-black'
                                                                : 'bg-green-500'
                                        ...
                                        >
                                            ...
                                            {SECTOR_GROUPS.map((sector) => {
                                                const centerAngle = ((sector.startWheelIndex + 2.5) * 360) / WHEEL_NUMBERS.length
                                                ...
                                                return (
                                                    ...
                                                    >
                                                        <span
                                                            className="text-[24px] sm:text-[28px] font-black text-amber-950 drop-shadow-sm leading-none"
                                                        >
                                                            {sector.label}
                                                        </span>
                                                    </div>
                                        ...
                                        <div className={`absolute inset-[33%] rounded-full bg-[radial-gradient(circle,#ffe17b_10%,#ebb726_60%,#d99c10_100%)] border-2 border-yellow-700/80 shadow-inner flex items-center justify-center ${!isSpinning && highlightedWinningNumber !== null ? 'ring-4 ring-yellow-300/70 shadow-[0_0_20px_rgba(250,204,21,0.5)]' : ''}`} style={{ transform: `rotate(${-rotation}deg)` }}>
                                            <div className={`w-16 h-16 sm:w-20 sm:h-20 rounded-full shadow-xl flex flex-col items-center justify-center border-2 border-yellow-200 transition-all duration-500 ${!isSpinning && highlightedWinningNumber !== null
                                                ...
                                                {isSpinning ? (
                                                    ...
                                                    <>
                                                        <span className={`text-xl sm:text-2xl font-extrabold leading-none text-white ${highlightedWinningNumber !== null ? 'animate-bounce' : ''
                                                            }`}>{lastResult.number}</span>
                                                        <span
                                                            className={`text-[8px] sm:text-[10px] font-bold uppercase leading-none mt-0.5 ${lastResult.color === 'red'
                                                                ? 'text-red-200'
                                                    ...
                                                    <span className="text-2xl sm:text-3xl font-extrabold leading-none text-white">?</span>
                                                )}
                                            </div>
                                        </div>
                                    </motion.div>
                        ...
                        {!showWheel && (
                            <BettingBoard placeBet={placeBet} currentBet={selectedChip} bets={pendingSelections} />
                        )}
                    </div>
                    ...
                    {showWinnersDisplay && (
                        <div className=" rounded p-2 border border-slate-500">
                            <h3 className="text-lg font-semibold mb-4 flex items-center justify-between">
                                ...
                                <span className="text-sm font-mono text-amber-300">Spin #{lastSpinId ?? '--'}</span>
                            </h3>
                            <div className="space-y-2 max-h-96 overflow-y-auto">
                                {recentWinners.length === 0 ? (
                                    <p className="text-slate-500 text-center py-4">No recent winners</p>
                                ) : (
                                            ...
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    ...
                                                    <p className="text-xs text-slate-400">
                                                        Won on {getNumberColor(winner.winningNumber)} {winner.winningNumber}
                                                    </p>
                                                </div>
                                            </div>
                                            <div className="text-right">
                                                <p className="font-bold text-green-400">${winner.amount.toFixed(2)}</p>
                                                <p className="text-xs text-slate-400">
                        ...
                        <div className="h-16 px-2 sm:px-3 flex items-center justify-between gap-2">
                            <div className="h-12 px-1 rounded-md bg-sky-950/40 flex items-center border">
                                <div className="text-left leading-tight">
                                    <p className="text-base text-xs  rounded font-mono">{pendingSelections.length} Pending</p>
                                </div>
                            </div>
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                >
                                    <span className="text-sm sm:text-base font-semibold border-gray-200 px-3 rounded">Bet</span>
                                </button>
                                ...
                                >
                                    ...
                                    <span className="hidden sm:inline text-[11px] sm:text-xs font-medium">Back</span>
                                </button>
                                ...
                                <div className="h-12 px-2 rounded-md bg-emerald-950/45 text-slate-100 flex items-center gap-1 border">
                                    ...
                                    <span className="min-w-10 text-center text-base sm:text-lg font-bold leading-none">{selectedChip}</span>
                            ...
                            <div className="max-h-48 overflow-y-auto px-3 pb-3">
                                {pendingSelections.length === 0 && ticketedBets.length === 0 ? (
                                    ...
                                    <div className="space-y-2">
                                        {pendingSelections.length > 0 && (
                                            <div>
                                                <p className="text-[10px] uppercase tracking-wide text-emerald-200/90 mb-1">Pending Bets</p>
                                                <div className="space-y-1">
                                                    {pendingSelections.map((bet, index) => (
                                                        <div key={`pending-${index}`} className="grid grid-cols-4 gap-2 text-xs border-b border-emerald-900 pb-1">
                                                            <span>{bet.amount.toFixed(2)}</span>
                                                            <span className="truncate">{bet.type === 'exact' ? `#${bet.value}` : bet.value}</span>
                                                            <span>{bet.odds}</span>
                                        ...
                                        {ticketedBets.length > 0 && (
                                            <div>
                                                <p className="text-[10px] uppercase tracking-wide text-emerald-200/90 mb-1">Ticketed Bets</p>
                                                <div className="space-y-1">
                                                    {ticketedBets.map((bet, index) => (
                                                        <div key={`ticketed-${index}`} className="grid grid-cols-4 gap-2 text-xs border-b border-emerald-900 pb-1">
                                                            <span>{bet.amount.toFixed(2)}</span>
                                                            <span className="truncate">{bet.type === 'exact' ? `#${bet.value}` : bet.value}</span>
                                                            <span>{bet.odds}</span>
                                                            <span>{(bet.amount * bet.odds).toFixed(2)}</span>
                                ...
                                {pendingSelections.length > 0 && (
                                    <div className="mt-2 text-xs text-emerald-100 flex justify-between">
                                        ...
                                        <span className="font-bold">ETB{pendingPotentialReturn.toFixed(2)}</span>
                                    </div>
                                )}
                                {ticketedBets.length > 0 && (
                                    <div className="mt-1 text-xs text-emerald-100/90 flex justify-between">
                                        <span>Ticketed Return</span>
                                        <span className="font-bold">ETB{ticketedPotentialReturn.toFixed(2)}</span>
                                    </div>
                                )}
                            </div>
                        </div>
