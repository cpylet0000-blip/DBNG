...
import { Gamepad2, User, Trophy, Upload, Download, Gift } from "lucide-react";
import "./NavBar.css";
...
const NavBar = () => {
  ...
  const itemClass = (path: string) =>
    `flex flex-col items-center justify-center gap-0.5 flex-1 transition-colors ${
      isActive(path) ? "text-blue-500" : "text-slate-400 hover:text-blue-400"
    }`;
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 border-t border-slate-800 bg-[#141415]/95 backdrop-blur-sm">
      <div className="flex h-12 items-center justify-around px-1">
        ...
        <Link
          to="/shamo"
          className={`${itemClass("/shamo")} shamo-item ${isActive("/shamo") ? "shamo-active" : ""}`}
        >
